## distributions3 interface (for gamlss.dist) ---------------------
###################################################################
###################################################################
###################################################################
###################################################################
## generator function for S3 distribution object
## based on all GAMLSS families provided in gamlss.dist
...
###################################################################
###################################################################
## standard S3 distributions3 methods

mean.GAMLSS <- function(x, ...) {
  stopifnot(requireNamespace("gamlss.dist"))
  f <- .gamlss_family(x)
  if (is.null(f$mean)) warning("the mean is not defined for ", f$family[1], " family", "\n")
  else{
...
variance.GAMLSS <- function(x, ...) {
  stopifnot(requireNamespace("gamlss.dist"))
  f <- .gamlss_family(x)
  if (is.null(f$variance)) warning("the variance is not defined for ", f$family[1], " family", "\n")
  else{
...
format.GAMLSS <- print.GAMLSS <- function(x, digits = pmax(3L, getOption("digits") - 3L), ...) {
...
#######################################################################
#######################################################################
## S3method(scoringRules::crps, GAMLSS)
## manually match gamlss.dist distributions with scoringRules
## distributions (mostly following base R)
# crps.GAMLSS <- function(y, x, drop = TRUE, elementwise = NULL, ...) {
#   stopifnot(requireNamespace("scoringRules") && requireNamespace("distributions3"))
#   FUN <- switch(attr(y, "family")[1L],
...
#     stop(sprintf("crps() not supported yet for the '%s' family", family)) ## FIXME: scoringRules also has gpd, gev, lapl, cnorm, tnorm, ...
#   )
